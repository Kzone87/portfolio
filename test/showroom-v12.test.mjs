...
const demos = [
  ...
  { route: 'mini-labs/', html: 'mini-labs/index.html', app: 'mini-labs/app.js', controls: ['content-run','extract-run','automation-run','security-run','release-run'], results: ['content-output','extract-output','automation-output','security-output','release-output'], technicalUtility: true }
];
...
test('corporate website has complete navigation, responsive controls and a working inquiry surface', async () => {
  const pages = {
    ...
    playbook: await load('nexa-tech-service/playbook.html'),
  ...
  for (const html of Object.values(pages)) {
    ...
    for (const label of ['회사소개','서비스','이용 고객','이용 상황','상담']) assert.ok(visibleText(html).includes(label), `corporate page missing label ${label}`);
...
test('customer label dictionary covers the internal states that appear in demos', () => {
  for (const label of ['처리 보류','상품 준비 중','관리자 확인 필요','출동 중','현장 작업 중','수정 요청','담당자 확인 필요','검수 담당자']) assert.match(customerUi, new RegExp(label));
});
