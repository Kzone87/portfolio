...
test('NEXA publishes seven complete customer-facing corporate pages', async () => {
...
test('homepage sells operational outcomes to diverse B2B buyers without fake proof', () => {
  ...
  for (const phrase of ['멈추기 전의 운영','생산·인쇄·제조 현장','유통·프랜차이즈·다지점','기업·교육·공공 사무환경','장비 공급사·서비스 파트너','예방 유지보수','장애·긴급 지원','설치·이전·변경','다사업장 운영','장비 수명주기 관리','SERVICE LEVELS','TRUST & GOVERNANCE','SERVICE PLAYBOOK','SERVICE PERFORMANCE','NEXA FIELD OPS']) assert.match(html, new RegExp(phrase));
...
test('industry page gives multiple buyer segments a recognizable entry point', () => {
  ...
  for (const phrase of ['생산·인쇄·제조 현장','유통·프랜차이즈·다지점','기업·교육·공공 사무환경','장비 공급사·서비스 파트너','물류·운영 거점','성장 중인 중소·중견기업','주요 고민','추천 구조','확인 지표']) assert.match(html, new RegExp(phrase));
});
...
test('case library is explicitly fictional and uses challenge-approach-outcome reasoning', () => {
  ...
  for (const phrase of ['PORTFOLIO SCENARIO','CHALLENGE','APPROACH','OPERATIONAL EFFECT']) assert.match(html, new RegExp(phrase));
...
test('service playbook proves operational competence with tangible artifacts and maturity', () => {
  const html = htmlByPage['playbook.html'];
  for (const phrase of ['SERVICE INTAKE','PREVENTIVE CHECK','FIELD REPORT','FOLLOW-UP QUEUE','SERVICE PERFORMANCE','SERVICE MATURITY','Predictive Ready','NEXA FIELD OPS']) assert.match(html, new RegExp(phrase));
...
test('case blueprints show discovery design output and review and connect to live operations', () => {
  const html = htmlByPage['cases.html'];
  for (const phrase of ['CASE BLUEPRINT','DISCOVERY','DESIGN','OUTPUT','REVIEW','Service Playbook']) assert.match(html, new RegExp(phrase));
  assert.match(html, /\.\.\/field-service-ops\//);
});
