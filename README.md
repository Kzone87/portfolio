...
이 저장소는 공개 포트폴리오 허브입니다. 과거에는 HTML/CSS/JS 학습작과 복사된 프로젝트 코드, Express API 실험을 한 저장소에 함께 보관했지만, 현재는 대표 프로젝트를 설명하고 각 원본 저장소로 연결하는 역할만 담당합니다.
...
- **Customer Map Planner** (`Kzone87/geocoding`) — Excel 거래처 데이터를 Kakao Maps에 지오코딩하고, 거리 기반 클러스터링·라벨 배치·A4 출력을 수행하는 업무 도구. GitHub Actions 회귀 테스트와 Pages 데모 배포를 사용합니다.
