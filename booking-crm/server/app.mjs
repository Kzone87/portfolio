...
export function createBookingApp(options={}){
  ...
  const handler=async(req,res)=>{
    try{
      ...
      if(path==='/ready'){const ready=store.health();json(res,200,{ready:ready.quick_check==='ok'});return}
      ...
      if(req.method==='POST'&&path==='/api/auth/logout'){const session=requireMutation(req,res,['STAFF','ADMIN']);if(!session)return;store.logout(session.id);json(res,200,{ok:true},{'set-cookie':'booking_session=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0'});return}
