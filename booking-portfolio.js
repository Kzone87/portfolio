function bootBookingPortfolio(){
  ...
  article.innerHTML=`
    ...
    <div class="booking-portfolio-preview" aria-hidden="true">
      ...
      <div class="booking-live-frame-wrap" data-live-preview="booking"><iframe data-preview-src="./booking-crm/" title="BOOKING CRM 실제 공개 화면 미리보기" loading="lazy" tabindex="-1" aria-hidden="true"></iframe><span class="booking-live-badge"><i></i> ACTUAL LIVE SCREEN</span></div>
    </div>`;
  anchor.parentNode.insertBefore(article,anchor);
  window.KZONE_PERFORMANCE_GATE?.scan(article);
  ...
  if(!document.getElementById('booking-portfolio-style')){
    const style=document.createElement('style');style.id='booking-portfolio-style';style.textContent=`
      ...
      @media(max-width:760px){.booking-portfolio-showcase{padding:22px;margin:22px 0 34px;border-radius:20px}.booking-portfolio-preview{display:none}.booking-portfolio-proof{grid-template-columns:1fr 1fr}.booking-portfolio-copy h3{font-size:31px}.booking-portfolio-actions a{width:100%;text-align:center;justify-content:center}}
      @media(prefers-reduced-motion:reduce){.booking-portfolio-actions a{transition:none}}
