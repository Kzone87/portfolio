...
await page.locator('button[data-view="market"]').click();await page.locator('#order-form input[name="customerName"]').fill('QA 실제고객');await page.locator('#order-form input[name="amount"]').fill('125000');await page.locator('#order-form button').click();await page.getByText('QA 실제고객').waitFor();for(const status of ['PICKING','SHIPPED','DELIVERED']){await page.locator(`[data-status="${status}"]`).click();await page.getByText(new RegExp(status)).waitFor()}console.log('PASS MARKET create -> PICKING -> SHIPPED -> DELIVERED');
