...
await page.getByRole('button',{name:'관리자에서 확인하기'}).click();
...
await page.getByRole('button',{name:'고객 메모 저장'}).click();
await page.getByRole('button',{name:'고객 예약'}).click();
...
await page.locator('.inquiry-item').filter({hasText:'문의고객'}).getByRole('button',{name:'연락완료'}).click();
await page.locator('.inquiry-item').filter({hasText:'문의고객'}).getByRole('button',{name:'종료'}).click();
