...
const surfaces = [
  {
    key: 'portfolio',
    url: `${root}/`,
    required: ['요구사항', '범위·견적', '개발·검수', '배포·인수인계'],
    selector: 'a[href*="project-inquiry"]'
  },
  {
    key: 'tech-service',
    url: `${root}/nexa-tech-service/`,
    required: ['NEXA TECH SERVICE', '정기점검', '현장지원'],
    selector: 'a[href="./contact.html"]'
  },
  {
    key: 'customer-service',
    url: `${root}/nexa-service-domain/`,
    required: ['서비스 요청 조회', '접수번호'],
    selector: '#lookup-form'
  },
  {
    key: 'service-operations',
    url: `${root}/field-service-ops/`,
    required: ['NEXA SERVICE OPERATIONS', '배차'],
    selector: '#dispatch-workspace'
  }
];
  ...
  for (const viewport of viewports) {
    ...
    try {
      ...
      for (const required of surface.required) {
        if (!text.includes(required)) throw new Error(`${surface.key}/${viewport.name}: missing copy ${required}`);
      }
...
// Customer Service: exercise the actual public demo OTP flow on the deployed page.
{
...
// Operations: exercise all commercial workspace views on the deployed page.
{
  const context = await browser.newContext({ viewport: { width: 1440, height: 1000 }, locale: 'ko-KR', timezoneId: 'Asia/Seoul' });
  const page = await context.newPage();
  try {
